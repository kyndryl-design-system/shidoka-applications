...
Do **not** create or rely on hand-authored example stories for each pattern (e.g. "inbox-side-drawer-example"). The LLM must **infer** how to use and consume the Shidoka Design System from: (1) the CEM-derived design-system context, (2) this considerations file, and (3) the page-template-builder layout and spacing rules. Generate stories that follow those rules so layout, spacing, and component usage are correct without targeted example files.
...
Story UI is vetted primarily as a **page/template builder**. For prompts like "build a page with ui-shell, header, local nav, footer, data table, side drawer", follow these rules so the model does not overload:
...
Full reference: `story-ui/docs/patterns/page-template-builder.md` (or `story-ui-docs/page-template-builder.md` when generated).

## Layout patterns

- Page/template: follow "Page / template builder" above; see `story-ui/docs/patterns/page-template-builder.md` for the complete pattern.
- Constraints: see `story-ui/docs/guidelines/constraints.md`.

## Template / page generator

Generated stories serve as copy-pasteable templates. For full-page prompts, apply the layout order and spacing above and produce complete, working examples that use only registered components with correct attributes and imports.
