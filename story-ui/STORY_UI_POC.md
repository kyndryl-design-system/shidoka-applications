...
- **story-ui.config.js** – Web Components (Shidoka), `kyn-*` components, Claude, generated stories in `src/stories/generated/`
- **story-ui-considerations.md** – Shidoka-specific guidance for the AI
...
- **Scripts**
  ...
  - `npm run storybook-with-ui` – run both (Storybook + Story UI)
...
- **story-ui.config.js** – framework, paths, `importExamples`, `layoutRules` (main padding, table wrapper, full-page shell order), LLM provider, `storyPrefix`, etc.
- **story-ui-considerations.md** – design system rules and **Shidoka-only** constraint: generated code must use only existing Shidoka components (kyn-_); no `lit/_` subpath imports (e.g. no lit/ui-shell, lit/data-table) or invented tags (my-\*). See “CRITICAL: Shidoka-only components” there.
- **story-ui/docs/** – extra docs loaded into AI context; **patterns/page-structure.md** gives a full-page layout example to improve generated layout.
