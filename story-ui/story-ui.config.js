...
export default {
  ...
  // Optional: path to design system docs (story-ui-docs/). When set, the server may load
  // design-system-context.md + considerations.md from here and include them in the context
  // sent to the LLM. Run "npm run generate-component-registry" to populate story-ui-docs/.
...
When the user or system provides "considerations" or design-system context (e.g. design-system-context.md, considerations.md), treat it as authoritative: only use tags, attributes, slots, and import paths listed there.
For full-page/template prompts (ui-shell, header, local nav, footer, data table, side drawer): follow the Page/template builder rules—fixed shell order, main padding (required), table wrapper, side drawer inside main with kyn-button slot="anchor", and spacing tokens. Generate the requested row count and exact button label (e.g. OPEN SESAME).
...
7. MAIN PADDING (required): Every <main> MUST have style="padding: var(--kd-page-gutter, 1rem);" so content has visible left/right indentation. Never omit this.
8. Side drawer: place inside <main>; use kyn-button slot="anchor". Do NOT add the open attribute unless the user asks for the drawer open by default.
9. If the user provides JSON data, use .map() to render rows dynamically.
