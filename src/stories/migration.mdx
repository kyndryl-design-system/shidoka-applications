...
- New Properties
  ...
  - Optional `valueFormat` on DatePicker and DateRangePicker (`'iso'` | `'dateFormat'`, default `'iso'`).
    - Default remains UTC ISO strings in `on-change.detail.dates` (no breaking change).
    - Use `valueFormat="dateFormat"` to emit calendar strings matching `dateFormat` (e.g. `Y-m-d` → `2026-06-01`), which avoids timezone day-shifts from local midnight.
    - Query Builder `dataType: 'date'` fields opt into `dateFormat` mode automatically.
- Event handling
