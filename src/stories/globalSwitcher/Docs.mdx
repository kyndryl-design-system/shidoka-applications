...
<kyn-header-link href="javascript:void(0)" full-width-flyout>
  ...
  >
    <kyn-tab
      ...
      fill-width
      selected
      style="width: var(--global-switcher-tab-width);"
...
For dynamic tab labels, the Global Switcher pattern can measure the rendered
service tabs and apply the widest tab width to the slotted `kyn-tab` hosts. Keep
that behavior scoped to the Global Switcher pattern; default `kyn-tabs` continue
to size naturally to their label text. Use `fill-width` on only the tabs that
the pattern sizes so the internal tab button fills the measured host width.
