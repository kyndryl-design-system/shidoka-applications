...
Add `class="global-switcher-nav"` to `kyn-header-nav` — that activates Global Switcher flyout framing inside the header components (`:host(.global-switcher-nav)` / `:host([global-switcher-nav])` in `headerNav.scss` / `headerLink.scss`).
