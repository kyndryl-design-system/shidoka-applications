...
import splitViewPatternStyles from './splitViewPattern.scss?inline';
...
/**
 * Storybook-only layout helper for the Split View pattern (not part of the public `kyn-*` API).
 * Uses `kyn-divider` with `drag-handle` for resize affordances.
 */
@customElement('split-view-pattern')
export class SplitViewPattern extends LitElement {
  static override styles = unsafeCSS(splitViewPatternStyles);
  ...
  override connectedCallback() {
    ...
    this._resizeObserver = new ResizeObserver(() => this._syncFromContainer());
  ...
  private _clampLeft(left: number) {
    ...
    if (this.panes === 2) {
      const maxLeft = Math.max(MIN_PANE_PX, total - DIVIDER_PX - MIN_CENTER_PX);
    ...
    return html`
      <div class="split-view" part="split-view">
        ...
        <div
          ...
          @pointerdown=${(e: PointerEvent) => this._onDividerPointerDown(1, e)}
        >
          <kyn-divider
            vertical
            drag-handle
...
declare global {
  interface HTMLElementTagNameMap {
    'split-view-pattern': SplitViewPattern;
  }
}
