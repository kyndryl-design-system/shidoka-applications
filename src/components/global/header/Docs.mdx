...
<Controls
  of={Stories.WithCategorizedNav}
  exclude={['rootUrl', 'appTitle', 'autoOpenFlyout']}
/>
...
### Width behavior for 3+ columns

- Default behavior: 3+ columns stretch to fill the flyout width with equal-width columns.
- Opt-in fixed widths: add `fixed-column-widths` on `kyn-header-categories`.
...
<kyn-header-categories
  ...
  maxColumns="4"
  fixed-column-widths
  style="--kyn-header-category-column-width: 280px;"
>
  ...
</kyn-header-categories>
```
