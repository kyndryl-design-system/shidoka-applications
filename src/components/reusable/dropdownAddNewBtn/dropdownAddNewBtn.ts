...
import DropdownScss from './dropdownAddNewBtn.scss';
import './dropdownAddNewBtnOption';
import '../tag';
...
import addIcon24 from '@carbon/icons/es/add/16';
import '../textInput';
import '@kyndryl-design-system/shidoka-foundation/components/button/button';
 ...
 */
@customElement('kyn-dropdown-addnew-btn')
export class DropdownAddNewBtn extends LitElement {
   ...
   * @ignore
   */
  @queryAssignedElements({ selector: 'kyn-dropdown-addnew-btn-option' })
  options!: Array<any>;
   ...
   */
  @queryAssignedElements({
    selector: 'kyn-dropdown-addnew-btn-option[selected]',
  })
  ...
  @query('.add-new')
  addNewInputEl!: HTMLInputElement;
  ...
  override render() {
    const addNewInput = html`
      <div class="add-new">
        <kyn-text-input
          name="textInput"
          placeholder="Add new option"
          caption=""
          @on-input=${(e: any) => console.log('on-input', e)}
          @blur=${(e: any) => this._handleAddNew(e)}
        >
          Text input
        </kyn-text-input>
      </div>
              ...
              ${this.addNewOption
                ? html`
                    <kyn-dropdown-addnew-btn-option
                      value=${this.addNewOption}
                      ?selected=${this.value === this.addNewOption}
                    >
                      ${this.addNewOption}
                    </kyn-dropdown-addnew-btn-option>
                  `
    ...
    `;
    const ddOrInput = this.renderInput ? addNewInput : dropdown;
    const addNewBtn = this.renderInput
      ? null
      : html` <kd-button
          style="max-height: 24px;"
          size="small"
          kind="secondary"
          @on-click=${this.handleAddBtnClicked}
        >
          <kd-icon .icon=${addIcon24}></kd-icon>
        </kd-button>`;
    return html`<span class="flex-box">${ddOrInput} ${addNewBtn}</span>`;
  }
  ...
  addNewOption = '';
  private _handleAddNew(e: any) {
    this.renderInput = false;
    console.log('_handleAddNew  : this.options', this.options);
    this.value = e.target.value;
    this.text = e.target.value;
    this.addNewOption = e.target.value;
  ...
  private handleListBlur(e: any) {
    ...
    if (
      e.relatedTarget &&
      e.relatedTarget.localName !== 'kyn-dropdown-addnew-btn-option'
    ) {
      this.open = false;
    }
  ...
  private _updateSelectedOptions() {
    console.log('_updateSelectedOptions : this.value', this.value);
  ...
  private _handleClick(e: any) {
    console.log('_handleClick _handleClick_handleClick_handleClick');
    if (e.detail.value === 'selectAll') {
      ...
      this._validate(true, false);
    } else if (e.detail.value === 'newOption') {
      this.updateValue(e.detail.value, e.detail.selected);
      this.renderInput = true;
      console.log('this.addNewInputEl', this.addNewInputEl);
      // this.addNewInputEl.focus();
    } else {
      this.updateValue(e.detail.value, e.detail.selected);
      this.assistiveText = 'Selected an item.';
    }
  ...
  private _handleBlur(e: any) {
    ...
    if (
      !relatedTarget ||
      (relatedTarget.localName !== 'kyn-dropdown-addnew-btn-option' &&
        relatedTarget.localName !== 'kyn-dropdown-addnew')
    ) {
      this.open = false;
    }
  }
  ...
  override updated(changedProps: any) {
    ...
    if (changedProps.has('value')) {
      ...
      const AllOptions: any = Array.from(
        this.querySelectorAll('kyn-dropdown-addnew-btn-option')
      );
  ...
  private _updateOptions() {
    ...
    console.log('_updateOptions : this', this);
    console.log('_updateOptions : Options', Options);
...
declare global {
  interface HTMLElementTagNameMap {
    'kyn-dropdown-addnew-btn': DropdownAddNewBtn;
  }
}
