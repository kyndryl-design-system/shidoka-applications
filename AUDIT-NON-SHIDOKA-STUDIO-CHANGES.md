# Audit: Code modifications outside `shidoka-studio/` and `packages/@kyndryl-design-system/shidoka-studio/`

**Scope:** All changes vs `main` that are not under `shidoka-studio/` or `packages/@kyndryl-design-system/shidoka-studio/`.  
...
| **.eslintrc.json**                                          | Overrides for `shidoka-studio/scripts/**`, `server/**`, `__tests__/**`, `packages/@kyndryl-design-system/shidoka-studio/bin/**` (node env).                              | So Shidoka Studio JS (scripts, server, tests) lint with Node globals; no component change.                                                                                               |
...
| **src/stories/boilerplate/workspaceSwitcherBoilerplate.js** | New file: default workspace data + `setupWorkspaceSwitcher()`.                                                                                                           | Optional/legacy; Shidoka Studio content and canonical page stories use the self-contained pattern (inline data + wiring in the same file). No component or pattern depends on this file. |
...
- **Keep:** All Shidoka Studio–specific config (Cursor, ESLint, CI build/test, gitignore, husky, package scripts + MCP SDK, vitest project), Storybook story globs/sidebar that support generated stories. Workspace switcher boilerplate file is optional (canonical pattern is self-contained in stories and Shidoka Studio content).
