...
- **kyn-side-drawer:** Place **inside** `<main>`. Use `<kyn-button slot="anchor" kind="tertiary">Label</kyn-button>` as the trigger (user's label e.g. "OPEN SESAME"). **Always set `titleText`** (e.g. `titleText="Drawer"` or `titleText="Sidebar"`) so the drawer panel shows a visible header/title; optionally set `labelText`. **WRONG:** kyn-side-drawer without titleText (drawer looks broken). **CORRECT:** kyn-side-drawer with titleText="Drawer" (or similar). Use `size` (sm, md, lg). **Do not add the `open` attribute** unless the user explicitly asks for the drawer to be open by default; the drawer is closed by default and opens on anchor click.
...
**Table headers required:** Every table must include a **header row**: `kyn-thead` with one `kyn-header-tr` and one `kyn-th` per column with **text content** (e.g. `<kyn-th>Name</kyn-th>`, `<kyn-th>Status</kyn-th>`). Never emit a table with only `kyn-tbody` or an empty thead. **WRONG:** table without kyn-thead or with empty kyn-th. **CORRECT:** kyn-thead > kyn-header-tr > kyn-th with visible column labels.
...
1. **Shell order (fixed):** Inside **kyn-ui-shell**: **kyn-header** → (optional **kyn-local-nav**) → **main** → **kyn-footer**. Never omit header or footer for a full page. **kyn-footer** defaults to copyright text "Copyright © {year} Kyndryl Inc. All rights reserved." — omit the `slot="copyright"` unless the user asks for custom copyright. **Header flyouts:** All four flyouts (workspace, notifications, help, user) must have **slot="button"** filled — workspace = label + chevron; notifications/help/user = **icon SVG** (bell, help circle, user avatar) so the header shows visible anchors. **Local nav:** If you include kyn-local-nav, each **kyn-local-nav-link** (level 1) must have **slot="icon"** with a 16px icon (placeholder SVG is fine); CEM requires it for level 1.
...
4. **Data table:** Wrap in a scrollable div: `style="overflow: auto; max-height: 360px; border: 1px solid var(--kd-color-border, #e0e0e0); border-radius: 4px;"` with **kyn-table-container** > **kyn-table** inside. **Include a header row:** kyn-thead > kyn-header-tr > one kyn-th per column with text (e.g. Name, Status). Use **kyn-thead**, **kyn-header-tr**, **kyn-th**, **kyn-tbody**, **kyn-tr**, **kyn-td** only—never native table elements.
5. **Side drawer:** Place **inside** `<main>`. Trigger = `<kyn-button slot="anchor" kind="tertiary">USER_LABEL</kyn-button>` (e.g. "OPEN SESAME"). **Set `titleText` on kyn-side-drawer** (e.g. titleText="Drawer") so the drawer panel has a visible title. Put drawer content (e.g. list) after the button. **Do not add `open`** unless the user explicitly asks for the drawer open by default.
...
**CRITICAL — They are separate; do not nest:** **Global switcher** lives in **kyn-header-nav** (left slot). **Workspace switcher** lives in a **kyn-header-flyout** that is inside **kyn-header-flyouts** (right side). **Never put kyn-workspace-switcher or a workspace flyout inside kyn-header-nav.** The header has two direct children for nav/flyouts: (1) kyn-header-nav = global switcher only (kyn-header-link + kyn-header-category + links), (2) kyn-header-flyouts = workspace flyout + notifications + help + user. Putting the workspace switcher inside the global nav breaks the UI.
...
1. **Placement:** Use `kyn-workspace-switcher` inside **kyn-header-flyout** (which is inside **kyn-header-flyouts**). Not inside kyn-header-nav. Set **hide-menu-label** and **hide-back-button** on the flyout. Put a **span** (or element) with `id="workspace-trigger-label"` in the flyout’s `slot="button"` for the trigger label. **WRONG:** `label="Workspace"` with only kyn-workspace-switcher and no slot="button" content. **CORRECT:** slot="button" has the trigger (workspace name + chevron); use hide-button-label.
